...
app.post("/update-profile", verifyJWT, (req, res) => {
    ...
    try {
        ...
        return res.json(500).json({ error : "Provide full social links with http(s) included"});
    }
...
app.post("/delete-comment", verifyJWT, (req, res) => {
    ...
    Comment.findOne({ _id })
    .then(comment => {
        if( ( comment.commented_by && user_id == comment.commented_by ) || user_id == comment.blog_author ){
            deleteComments(_id);
