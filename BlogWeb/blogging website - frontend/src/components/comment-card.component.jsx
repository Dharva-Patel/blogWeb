...
const CommentCard = ({ index, leftVal, commentData}) => {
    ...
    const handleReplyClick = () => {
        if(!access_token){
            toast.error("Login to add a reply");
        }
    ...
    const LoadMoreRepliesButton = () => {
        ...
        let LoadMoreReplybutton = <button 
        onClick={() => loadReplies({ skip: index-parentIndex, currentIndex: parentIndex })}
        className="text-dark-grey p-2 px-3 hover:text-black hover:underline flex items-center gap-2">
            Load More Replies
        </button>
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">

                    {
                        ...
                        <button className="text-dark-gey p-2 px-3 hover:underline flex items-center gap-2" onClick={hideReplies}>
                        <i className="fi fi rs-comment-dots"></i>
                        Hide Reply</button>
                        : 
                        <button className="text-dark-gey p-2 px-3 hover:underline" onClick={loadReplies}>
                        <i className="fi fi rs-comment-dots"></i>
                        {children.length} Reply</button>
                    }
